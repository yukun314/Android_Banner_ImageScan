...
public class GestureImageViewTouchListener implements OnTouchListener {
			...
			if (!tapDetector.onTouchEvent(event)) {
				...
				if (event.getAction() == MotionEvent.ACTION_UP) {
					...
					if (event.getPointerCount() > 1) {
						...
						// System.out.println("GestureImageViewTouchListener touched:"+touched+"  multiTouch:"+multiTouch);
						if (!touched) {
							touched = true;
							last.x = event.getX();
							last.y = event.getY();
	 ...
	 *   ���ݵ�ǰͼƬ���ұ�Ե���ÿ���ק״̬
	 */
	//FIXME ������һ����Ե�������෴���򻬶� ��Ȼ��viewPage��ȡ�����¼�
	private void checkDragable(float dx) {
//		System.out.println("dx:"+dx);
		int effectiveWidth = Math.round((float) imageWidth * currentScale);
		boolean can = effectiveWidth <= displayWidth;
		boolean left = next.x <= boundaryLeft;
		boolean right = next.x >= boundaryRight;
		if(left || right || can){
			stopDrag();
		}else{
			startDrag();
